#!/usr/bin/env python3
"""
Hybrid Canonical CSV Replayer for ENERGY/UTILITY ZONE
----------------------------------------------------
- Zone: ENERGY (~50 devices concept)
- Giữ nguyên toàn bộ logic gốc, chỉ thêm "zone": "energy"
Usage:
  python mqtt_csv_replayer_energy.py --indir datasets --broker emqx --port 1883
"""
...
def device_thread(device_name: str, csv_path: str, broker: str, port: int,
    ...
    if not ts_col:
        ...
        seconds = _parse_timestamp_series(df[ts_col]); base_interval = _median_interval(seconds)
    ...
    for i in range(len(df)):
        ...
        intervals.append(max(delta / max(speed_factor, 1e-6), min_interval))
    ...
    try:
        while True:
            ...
            time.sleep(intervals[i]); i = (i + 1) % len(df)
    finally:
...
def main():
    ...
    print("CSV Replayer (Energy Zone) Starting...")
