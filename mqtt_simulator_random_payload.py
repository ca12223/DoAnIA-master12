#!/usr/bin/env python3
"""
Hybrid Canonical CSV Replayer (Realistic Timing, Randomized Device-Specific Payloads)
------------------------------------------------------------------------------------
- Replays each device's CSV at the *original timing* between rows (with optional
  speed-up/slow-down).
- Uses CSV only to determine timing and message count.
- Publishes device-specific randomized payloads:
    {"timestamp", "value", "client_id"}
- Loops continuously per device.
- Filters only rows that represent a publish (mqtt.msgtype == 3 or contains "Publish Message").

Usage:
  python mqtt_csv_replayer.py --indir datasets --broker localhost --port 1883 \
    --speed-factor 1.0 --min-interval 0.05

Notes:
- Timestamp in payload = current UTC time.
- Value = random float in device-specific range.
- client_id = username from the DEVICES tuple.
"""
...
# -----------------------------------------------------------------------------
# Device configuration: (Name, CSV filename, username)
# -----------------------------------------------------------------------------
DEVICES = [
    ...
    ("AirQuality", "air-quality_gotham.csv", "sensor_air"),
    ("CoolerMotor", "cooler-motor_gotham.csv", "sensor_cooler"),
    ("DistanceSensor", "Edge-IIoTset_distance_sensor.csv", "sensor_distance"),
    ("FlameSensor", "Edge-IIoTset_flame_sensor.csv", "sensor_flame"),
    ("PhLevel", "Edge-IIoTset_PhLv.csv", "sensor_ph"),
    ("SoilMoisture", "Edge-IIoTset_soil_moisture.csv", "sensor_soil"),
    ("SoundSensor", "Edge-IIoTset_sound_sensors.csv", "sensor_sound"),
    ("WaterLevel", "Edge-IIoTset_WaterLV.csv", "sensor_water"),
    ("HydraulicSystem", "hydraulic-system_gotham.csv", "sensor_hydraulic"),
    ("PredictiveMaintenance", "predictive-maintenance_gotham.csv", "sensor_predictive"),
]
...
# -----------------------------------------------------------------------------
# Device-specific random value generator
# -----------------------------------------------------------------------------
...
def device_thread(device_name: str, csv_path: str, broker: str, port: int,
    ...
    client_id = f"{username or device_name}-replayer"
    ...
    while not connected:
        try:
            ...
            print(f"[{device_name}] Connection failed, retrying in 5s: {e}")
            time.sleep(5)

    # load csv
    try:
        df = pd.read_csv(csv_path, low_memory=False)
        print(f"[{device_name}] Loaded {len(df)} rows from {csv_path}")
    except Exception as e:
        print(f"[{device_name}] Error loading CSV: {e}")
    ...
    if not ts_col:
        print(f"[{device_name}] No timestamp column found; using 1.0s default interval.")
        ...
        while True:
            ...
            if _is_publish(row, msg_col):
                ...
                try:
                    ...
                    print(f"[{device_name}] Row {i+1}/{len(df)} → published: {payload}")
                except Exception as e:
                    print(f"[{device_name}] Publish error: {e}")
            else:
                print(f"[{device_name}] Row {i+1}/{len(df)} skipped (msgtype not publish)")
...
def main():
    parser = argparse.ArgumentParser(description="CSV Replayer with Realistic Timing & Device Random Payloads")
