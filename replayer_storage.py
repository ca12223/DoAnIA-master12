...
TIMESTAMP_CANDIDATES = [
    "timestamp", "ts", "time", "frame.time_epoch", "frame.time_relative", "Time", "SniffTimestamp"
]
...
def _parse_timestamp_series(ts: pd.Series) -> pd.Series:
    if pd.api.types.is_numeric_dtype(ts):
        s = ts.astype(float)
        if s.dropna().median() > 1e12: s = s / 1000.0
        return s
...
def _is_publish(row: pd.Series, msgtype_col: Optional[str]) -> bool:
    if not msgtype_col or msgtype_col not in row or pd.isna(row[msgtype_col]): return True
    v = row[msgtype_col]
    try:
        if str(v).strip().isdigit(): return int(v) == 3
...
def random_value_for_device(username: str) -> float:
    ranges: dict[str, Tuple[float, float]] = {
        "sensor_temp": (15.0, 40.0), "sensor_light": (0.0, 2000.0), "sensor_hum": (20.0, 90.0),
        "sensor_motion": (0, 1), "sensor_co": (0.0, 50.0), "sensor_smoke": (0.0, 10.0),
        "sensor_fanspeed": (500, 3000), "sensor_door": (0, 1), "sensor_fan": (500, 2500),
        "sensor_air": (0.0, 150.0), "sensor_cooler": (0.5, 5.0), "sensor_distance": (1.0, 400.0),
        "sensor_flame": (0, 1), "sensor_ph": (5.5, 8.5), "sensor_soil": (5.0, 60.0),
        "sensor_sound": (30.0, 100.0), "sensor_water": (0.0, 300.0),
        "sensor_hydraulic": (50.0, 250.0), "sensor_predictive": (0.0, 1.0)
    }
    lo, hi = ranges.get(username, (0.0, 100.0))
    val = random.uniform(lo, hi)
    if hi - lo <= 5 or (lo == 0 and hi <= 1): return round(val, 3)
    elif hi <= 100: return round(val, 2)
    else: return round(val, 1)
...
def device_thread(device_name: str, csv_path: str, broker: str, port: int,
    ...
    if not ts_col:
        seconds = pd.Series(range(len(df)), dtype=float); base_interval = 1.0
    else:
        seconds = _parse_timestamp_series(df[ts_col])
        base_interval = _median_interval(seconds)
        if pd.isna(seconds).all(): seconds = pd.Series(range(len(df)), dtype=float); base_interval = 1.0
    ...
    for i in range(len(df)):
        ...
        else: delta = base_interval
        if not (delta > 0): delta = base_interval
        ...
        while True:
            ...
            if _is_publish(row, msg_col):
                ...
                try:
                    ...
                    print(f"[{ZONE}:{device_name}] published: {payload}")
                except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description=f"{ZONE.capitalize()} Zone CSV Replayer")
    parser.add_argument("--indir", default="datasets", help="CSV dataset folder")
    parser.add_argument("--broker", default="emqx", help="MQTT broker")
    parser.add_argument("--port", type=int, default=1883)
    parser.add_argument("--speed-factor", type=float, default=1.0)
    parser.add_argument("--min-interval", type=float, default=0.05)
    args = parser.parse_args()

    print(f"Starting {ZONE} zone replayer → {args.broker}:{args.port}")
    print("=" * 70)
    ...
    for name, fname, username in DEVICES:
        ...
        print(f"Started {name} → topic factory/{TENANT}/{name}/telemetry")
